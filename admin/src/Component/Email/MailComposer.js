...
import { useDataProvider,fetchUtils, fetchStart, fetchEnd } from 'react-admin';
...
const MailComposer = ({isOpen, selectedIds, onClose, resource, basePath, filterValues,campagneInit, shouldFetch, limit=1000}) => {
    ...
    const handleClickEnvoiTest = () => {
        ...
        /*setLoadingTest(true);
        dispatchTest(fetchStart()); // start the global loading indicator
            fetch(`campaigns/send-test/${campagne.originId}`, { method: 'GET' })
                .then(() => {
                    console.info('Comment approved');
                    console.info(`${apiUrl}/${campagne.originId}`);
                })
                .catch((e) => {
                ...
                .finally(() => {
                    ...
                    dispatchTest(fetchEnd()); // stop the global loading indicator
                    console.info('end')
                });
*/
    };
