const getKibanaFilter = ({ from, communitiesList }) => {
  ...
  if (communitiesList.lenght > 5) {
    console.error(
      'User manages more than 5 communities. The dashboard may display incomplete data.'
    );
  }
  const filterPattern = communitiesList
    .slice(0, 5)
    .map((c) => `( match_phrase:(community_name.keyword:'${encodeURI(c)}') )`)
    .join(',');
