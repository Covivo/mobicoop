...
class InteropProvider implements CarpoolStandardProviderInterface
{
    ...
    public function __construct(string $interopBaseUri, string $interopApiKey)
    {
    ...
    {
        $dataProvider = new DataProvider($this->interopBaseUri.self::RESSOURCE_MESSAGE);

        $headers = [
            'X-API-KEY' => $this->interopBaseUri,
        ];
        // Build the body
        $body['from'] = [];
        $body['to'] = [];
