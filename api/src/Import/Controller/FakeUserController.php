...
class FakeUserController extends AbstractController
{
    ...
    public function faker($number_users, $number_addresses, $min_lat, $min_lon, $max_lat, $max_lon, GeoSearcher $geoSearcher, EntityManagerInterface $entityManager)
    {
        ...
        while ($generated < $number_users) {
            ...
            $criteria->setDriver($role == Ad::ROLE_DRIVER || $role == Ad::ROLE_DRIVER_OR_PASSENGER);
            $criteria->setPassenger($role == Ad::ROLE_PASSENGER || $role == Ad::ROLE_DRIVER_OR_PASSENGER);
            ...
            if ($proposal->getType() == Proposal::TYPE_OUTWARD) {
                ...
                $criteriaReturn->setPassenger($role == Ad::ROLE_PASSENGER || $role == Ad::ROLE_DRIVER_OR_PASSENGER);
                if ($criteriaReturn->getFrequency() == Criteria::FREQUENCY_PUNCTUAL) {
