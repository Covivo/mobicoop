...
class ResultManager
{
    ...
    private function createMatchingResult(Proposal $proposal, Proposal $matchingProposal, array $matching, bool $return)
    {
        ...
        // REQUEST
        if (isset($matching['request'])) {
            $result->setCommonDuration(0);
            if (is_numeric($matching['request']->getDropOffDuration()) && is_numeric($matching['request']->getPickUpDuration())) {
                $result->setCommonDuration($matching['request']->getDropOffDuration() - $matching['request']->getPickUpDuration());
            }
        ...
        if (isset($matching['offer'])) {
            $result->setCommonDuration(0);
            if (is_numeric($matching['offer']->getDropOffDuration()) && is_numeric($matching['offer']->getPickUpDuration())) {
                $result->setCommonDuration($matching['offer']->getDropOffDuration() - $matching['offer']->getPickUpDuration());
            }
