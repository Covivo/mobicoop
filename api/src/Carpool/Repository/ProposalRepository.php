...
class ProposalRepository
{
     ...
     * @return Proposal[]
     */
    public function findByDate(\Datetime $date, User $user=null, $onlyOneWayOrOutward = false): array
    {
        $query = $this->repository->createQueryBuilder('p')
        ->join('p.criteria', 'c')
        ->where('(c.frequency = :punctualFrequency and c.fromDate = :date) or 
                    (c.frequency = :regularFrequency and c.fromDate <= :date and c.toDate >= :date) ')
